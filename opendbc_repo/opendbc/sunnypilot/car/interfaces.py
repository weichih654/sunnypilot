...
def _initialize_mazda(CP: structs.CarParams, CP_SP: structs.CarParamsSP, params_dict: dict[str, str]) -> None:
  if CP.brand == 'mazda':
    torque_interceptor = int(params_dict.get("MazdaTorqueInterceptor", 0)) == 1

    if torque_interceptor:
      CP_SP.flags |= MazdaFlagsSP.TORQUE_INTERCEPTOR.value
      CP_SP.safetyParam |= MazdaSafetyFlagsSP.TORQUE_INTERCEPTOR
      # the torque interceptor bypasses the stock EPS low-speed LKAS lockout,
      # allowing steering down to a standstill
      CP.minSteerSpeed = 0.
      # dashcamOnly would force a noOutput safety model, which blocks the
      # interceptor keepalive and makes it undetectable
